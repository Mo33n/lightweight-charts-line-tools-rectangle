(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.LightweightChartsLineToolsRectangle = {}, global.LightweightCharts, global.LightweightChartsLineToolsCore));
     ...
     */
    class LineToolRectanglePaneView extends lightweightChartsLineToolsCore.LineToolPaneView {
        ...
        _updateImpl(height, width) {
            ...
            const rectangleRendererData = {
                ...lightweightChartsLineToolsCore.deepCopy(options.rectangle),
            ...
            if (options.text.value) {
                ...
                const textRendererData = {
                    text: lightweightChartsLineToolsCore.deepCopy(options.text),
                    points: rectanglePoints,
                    toolDefaultHoverCursor: options.defaultHoverCursor,
                    toolDefaultDragCursor: options.defaultDragCursor,
        ...
        _addAnchors(renderer) {
            ...
            const diag1Cursor = sign < 0 ? lightweightChartsLineToolsCore.PaneCursorType.DiagonalNeSwResize : lightweightChartsLineToolsCore.PaneCursorType.DiagonalNwSeResize; // NE-SW or NW-SE
            const diag2Cursor = sign < 0 ? lightweightChartsLineToolsCore.PaneCursorType.DiagonalNwSeResize : lightweightChartsLineToolsCore.PaneCursorType.DiagonalNeSwResize; // NW-SE or NE-SW
            // --- DEFINE EACH ANCHOR POINT WITH ITS CANONICAL INDEX ---
            // 0: Top-Left (TL)
            const topLeft = new lightweightChartsLineToolsCore.AnchorPoint(minX, minY, 0, false, diag1Cursor);
            // 6: Top-Center (TC)
            const topCenter = new lightweightChartsLineToolsCore.AnchorPoint((minX + maxX) / 2, minY, 6, true, lightweightChartsLineToolsCore.PaneCursorType.VerticalResize);
            // 3: Top-Right (TR)
            const topRight = new lightweightChartsLineToolsCore.AnchorPoint(maxX, minY, 3, false, diag2Cursor);
            // 5: Middle-Right (MR)
            const middleRight = new lightweightChartsLineToolsCore.AnchorPoint(maxX, (minY + maxY) / 2, 5, true, lightweightChartsLineToolsCore.PaneCursorType.HorizontalResize);
            // 1: Bottom-Right (BR)
            const bottomRight = new lightweightChartsLineToolsCore.AnchorPoint(maxX, maxY, 1, false, diag1Cursor);
            // 7: Bottom-Center (BC)
            const bottomCenter = new lightweightChartsLineToolsCore.AnchorPoint((minX + maxX) / 2, maxY, 7, true, lightweightChartsLineToolsCore.PaneCursorType.VerticalResize);
            // 2: Bottom-Left (BL)
            const bottomLeft = new lightweightChartsLineToolsCore.AnchorPoint(minX, maxY, 2, false, diag2Cursor);
            // 4: Middle-Left (ML)
            const middleLeft = new lightweightChartsLineToolsCore.AnchorPoint(minX, (minY + maxY) / 2, 4, true, lightweightChartsLineToolsCore.PaneCursorType.HorizontalResize);
            const anchorData = {
    ...
    const RectangleOptionDefaults = {
        ...
        rectangle: {
            ...
            border: { radius: 0, width: 1, style: lightweightCharts.LineStyle.Solid, color: '#9c27b0' }, // default purple border
        },
        text: {
            ...
            box: {
                ...
                border: { color: 'rgba(0,0,0,0)', width: 0, radius: 0, highlight: false, style: lightweightCharts.LineStyle.Solid },
     ...
     */
    class LineToolRectangle extends lightweightChartsLineToolsCore.BaseLineTool {
        /**
        ...
        options = {}, points = [], priceAxisLabelStackingManager) {
            ...
            const finalOptions = lightweightChartsLineToolsCore.deepCopy(RectangleOptionDefaults);
        ...
        getShiftConstrainedPoint(pointIndex, rawScreenPoint, phase, originalLogicalPoint, allOriginalLogicalPoints) {
            ...
            if (pointIndex >= 0 && pointIndex <= 3) {
                ...
                return {
                    point: new lightweightChartsLineToolsCore.Point(rawScreenPoint.x, constrainedY),
                    snapAxis: 'price',
                };
            }
        ...
        updateCullingState() {
            ...
            const cullingState = lightweightChartsLineToolsCore.getToolCullingState(points, this, options.rectangle.extend, undefined, undefined, true // isAreaBased: true
            );
    ...
    exports.LineToolRectangle = LineToolRectangle;
    exports.LineToolRectanglePaneView = LineToolRectanglePaneView;

}));
//# sourceMappingURL=lightweight-charts-line-tools-rectangle.umd.js.map
